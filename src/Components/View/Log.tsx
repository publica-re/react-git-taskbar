...
class Log extends Git.Component<LogProps & Intl.WithTranslation, LogState> {
  ...
  @bind
  private onColumnClick(_event: React.MouseEvent, column: UI.IColumn): void {
  ...
  @bind
  renderRow(props?: UI.IDetailsRowProps): React.ReactElement | null {
    if (props) {
      return (
        <div
          ref={(instance): void => {
          ...
          onClick={(): void => this.setState({ currentFocus: props.item.oid })}
        >
  ...
  @bind
  renderContextMenuItems(): UI.IContextualMenuItem[] {
    const { t } = this.props;
    return [
      {
        key: 'ammend',
        ...
        onClick: (): void => {
          this.state.currentFocus && this.onAmend(this.state.currentFocus);
        },
      },
      {
        key: 'revert',
        ...
        onClick: (): void => {
          this.state.currentFocus && this.onRevert(this.state.currentFocus);
        },
      },
      {
        key: 'rebase',
        ...
        onClick: (): void => {
          this.state.currentFocus && this.onRebase(this.state.currentFocus);
        },
      },
      {
        key: 'new-branch',
        ...
        onClick: (): void => {
          this.state.currentFocus && this.onNewBranch(this.state.currentFocus);
        },
      },
      {
        key: 'diff',
    ...
    return (
      <React.Fragment>
        ...
        <UI.ContextualMenu
          ...
          onDismiss={(): void => this.setState({ currentFocus: undefined })}
