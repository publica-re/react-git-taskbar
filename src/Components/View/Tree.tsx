...
class Tree extends Git.Component<TreeProps & Intl.WithTranslation, TreeState> {
  ...
  @bind
  private async onDiscard(path: string): Promise<void> {
    const { file } = this.context.io;
    await file.discardChanges({ path });
  }

  @bind
  private async onDeleteFile(path: string): Promise<void> {
    const { file } = this.context.io;
    await file.remove({ path });
  }

  @bind
  private async onDeleteDirectory(path: string): Promise<void> {
    const { directory } = this.context.io;
    await directory.remove({ path });
  }

  @bind
  private async onUploadFile(path: string): Promise<void> {
    this.setState({ fileUploadDirectory: path });
    this.fileOpenRef.current?.click();
  }

  @bind
  private async onNewFile(path: string): Promise<void> {
    const { file } = this.context.io;
    const { t } = this.props;
  ...
  @bind
  private async onNewFolder(path: string): Promise<void> {
  ...
  @bind
  private async onRename(oldPath: string): Promise<void> {
    const { file } = this.context.io;
    const { t } = this.props;
    const fileName = await this.props.prompt(
      t('action.file.new-name'),
  ...
  @bind
  private async onMove(oldPath: string): Promise<void> {
    const { file } = this.context.io;
    const { t } = this.props;
  ...
  @bind
  private downloadData(fileName: string, data: BlobPart): void {
  ...
  @bind
  private async onDownloadFile(path: string): Promise<void> {
    const { file } = this.context.io;
    const content = await file.read({
      path: path,
      mode: 'binary',
    });
    if (content.type === 'success' && content.value !== undefined) {
      this.downloadData(pathUtils.basename(path), content.value);
    }
  }

  @bind
  private async onDownloadDirectory(path: string): Promise<void> {
    const { file, directory } = this.context.io;
    const zip = new JSZip();
    ...
    async function walkDir(
      ...
      let newFolder = oldFolder.folder(name);
      if (newFolder === null) newFolder = oldFolder;
        ...
        if (child.type === 'file') {
          ...
          if (fileData.type === 'success') {
            if (fileData.value !== undefined) {
              // here was a , false at the end, maybe keep it
              newFolder.file(fileName, fileData.value);
            }
          }
        } else {
  ...
  @bind
  private async onStageDirectory(path: string): Promise<void> {
    const { file, directory } = this.context.io;
  ...
  @bind
  private async onDropFile(directoryPath: string): Promise<void> {
    const { file } = this.context.io;
    const files = (window.event as DragEvent).dataTransfer?.files;
